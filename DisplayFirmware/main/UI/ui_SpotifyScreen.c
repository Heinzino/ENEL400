...
void ui_SpotifyScreen_screen_init(void)
{
    ...
    lv_obj_align(ui_ImageAlbumCover, LV_ALIGN_TOP_LEFT, 10, 70); // Start after header

    // Text container beside album
    lv_obj_t *textContainer = lv_obj_create(ui_SpotifyScreen);
    lv_obj_remove_style_all(textContainer);
    lv_obj_set_size(textContainer, 300, 60);
    lv_obj_align_to(textContainer, ui_ImageAlbumCover, LV_ALIGN_OUT_RIGHT_TOP, 10, 0);
    ...
    lv_obj_set_style_text_font(ui_LabelTrack, &lv_font_montserrat_24, 0);
    ...
    lv_obj_align(ui_ProgressBar, LV_ALIGN_BOTTOM_MID, 0, -50); // Good spot near bottom
    ...
    // === Play/Pause Button (Smaller & Raised) ===
    ui_ButtonPlayPause = lv_btn_create(ui_SpotifyScreen);
    lv_obj_set_size(ui_ButtonPlayPause, 50, 40);  // Smaller
